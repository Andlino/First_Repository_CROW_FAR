...
if(!dir.exists("../data_archive/varde_archive")) dir.create("../data_archive/varde_archive")

#set base url for looping -- this is more of a 'start' page for this task
start_url <- "http://polweb.varde.dk/SitePages/Startside.aspx"
...
com_links <- paste0("http://polweb.varde.dk", com_links)
...
for(cc in 1:length(com_links)){
  ...
  years_links <- paste0("http://polweb.varde.dk", years_links)
  ...
  for(yc in 1:length(years_links)){
    ...
    meet_links <- paste0("http://polweb.varde.dk", meet_links)
    ...
    for (mc in 1:length(meet_links)) {
      ...
      file.name <- paste0("../data_archive/varde_archive/", date, ".PDF")
      ...
      if (!file.exists(file.name)){
        ...
        while (ok == FALSE) {
          meeting_url2 <- tryCatch({                  
            read_html(paste0("http://polweb.varde.dk", meeting_url))
          },
          error = function(e) {problm <- TRUE
          Sys.sleep(2)
          e
        ...
        pdf_link <- paste0("http://polweb.varde.dk", pdf_link)
...
save(out, file = '../data_archive/varde_13-17.RData')
