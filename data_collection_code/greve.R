...
## Massimo G Losinno
## August 2019
## Scrape Greve city council referater
...
if(!dir.exists("../data_archive/greve")) dir.create("../data_archive/greve_archive")


#Getting the main url link in
master_url <- "http://polweb.nethotel.dk/Produkt/PolWeb/default.asp?p=greve"
master_url <- getURL(master_url) 
master_url <- read_html(master_url)

years <- as.character(13:17)
  ...
  for(yc in 1:length(years)){
    ...
    if(length(links) >= 1){
      for (mc in 1:length(links)) {
        ...
        file.name <- paste0("../data_archive/greve_archive/", date, ".RData")
        ...
        if (file.exists(file.name)){
          ...
          junior_url2 <- paste0("HTTP://polweb.nethotel.dk/Produkt/PolWeb/Sog/ShowAgenda.asp?p=greve&ID=",
...
save(out, file = '../data_archive/greve_13-17.RData')
