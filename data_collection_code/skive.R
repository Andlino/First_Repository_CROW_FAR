...
## Massimo G Losinno
## August 2019
## Scrape Skive city council referater
...
if(!dir.exists("../data_archive/skive")) dir.create("../data_archive/skive_archive")


#Getting the main url link in
master_url <- "http://polweb.nethotel.dk/Produkt/PolWeb/default.asp?p=skive"
  ...
  for(yc in 1:length(years)){
    ...
    if(length(links) >= 1){
      for (mc in 1:length(links)) {
        ...
        file.name <- paste0("../data_archive/skive_archive/", date, ".RData")
        ...
        if (file.exists(file.name)){
          ...
          junior_url2 <- paste0("HTTP://polweb.nethotel.dk/Produkt/PolWeb/Sog/ShowAgenda.asp?p=skive&ID=",
...
save(out, file = '../data_archive/skive_09-17.RData')
