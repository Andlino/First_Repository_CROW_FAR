...
## Massimo G Losinno
## August 2019
## Scrape Helsingør city council referater
...
if(!dir.exists("../data_archive/helsingoer_archive")) dir.create("../data_archive/helsingoer_archive")

#set base url for looping -- this is more of a 'start' page for this task
start_url <- "https://polweb.helsingor.dk/open/Forms/AllItems.aspx"
...
com_links <- com_links[-grep("2018", com_check)]

com_links <- paste0("https://polweb.helsingor.dk", com_links)
...
for(cc in 1:length(com_links)){
  ...
  years_links <- paste0("https://polweb.helsingor.dk", years_links)
  ...
  for(yc in 1:length(years_links)){
    ...
    meet_links <- paste0("https://polweb.helsingor.dk", meet_links)
    ...
    for (mc in 1:length(meet_links)) {
      ...
      file.name <- paste0("../data_archive/helsingoer_archive/", date, ".PDF")
      ...
      while (ok == FALSE) {
        meeting_url2 <- tryCatch({                  
          read_html(paste0("https://polweb.helsingor.dk", meeting_url))
        },
        error = function(e) {problm <- TRUE
        Sys.sleep(2)
        e
      ...
      pdf_link <- paste0("https://polweb.helsingor.dk", pdf_link)
...
save(out, file = '../data_archive/helsingoer_13-17.RData')
