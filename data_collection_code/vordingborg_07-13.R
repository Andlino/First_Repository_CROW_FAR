...
if(!dir.exists("../data_archive/vordingborg_07-13_archive")) dir.create("../data_archive/vordingborg_07-13_archive")


#Getting the main url link in
master_url <- "https://polweb.nethotel.dk/Produkt/PolWeb/default.asp?p=vordingborg07"
master_url <- getURL(master_url) 
master_url <- read_html(master_url)

years <- as.character(10:13)
  ...
  for(yc in 1:length(years)){
    ...
    if(length(links) >= 1){
      for (mc in 1:length(links)) {
        ...
        file.name <- paste0("../data_archive/vordingborg_07-13_archive/", date, ".RData")
        ...
        if (file.exists(file.name)){
          ...
          junior_url2 <- paste0("HTTP://polweb.nethotel.dk/Produkt/PolWeb/Sog/ShowAgenda.asp?p=vordingborg07&ID=",
...
save(out, file = '../data_archive/vordingborg_07-13.RData')
