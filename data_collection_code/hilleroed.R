...
if(!dir.exists("../data_archive/hilleroed")) dir.create("../data_archive/hilleroed_archive")


#Getting the main url link in
master_url <- "http://polweb.nethotel.dk/Produkt/PolWeb/default.asp?p=hilleroed"
  ...
  for(yc in 1:length(years)){
    ...
    if(length(links) >= 1){
      for (mc in 1:length(links)) {
        ...
        file.name <- paste0("../data_archive/hilleroed_archive/", date, ".RData")
        ...
        if (file.exists(file.name)){
          ...
          junior_url2 <- paste0("HTTP://polweb.nethotel.dk/Produkt/PolWeb/Sog/ShowAgenda.asp?p=hilleroed&ID=",
...
#Scraping two extra meetings not on the original website
extra_links <- c("https://www.hillerod.dk/indflydelse-og-politik/politik/dagsordener-og-referater-gammel/vis-dagsorden/?agendaId=314838", "https://www.hillerod.dk/indflydelse-og-politik/politik/dagsordener-og-referater-gammel/vis-dagsorden/?agendaId=315121")
extra_dates <- c("13-12-2017", "13-12-2017")

for(el in 1:2){
  
  print(paste("Working on extra link", el, "out of 2"))
  
  
  dates <- str_c(c(dates, extra_dates[el]))
  date <- make.unique(dates, "-")[length(dates)]
  ref_date <- dates[length(dates)]
  ...
  # Extract and clean up transcriptions
  ref <- meet_url %>% html_nodes('.accordion__item') %>% html_text(trim = T)

  ref <- str_squish(ref)
  
  items <- meet_url %>% html_nodes('.header-title') %>% html_text(trim = T)
...
save(out, file = '../data_archive/hilleroed_10-17.RData')
