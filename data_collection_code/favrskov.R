...
## Massimo G Losinno
## August 2019
## Scrape Favrskov city council referater
...
if(!dir.exists("../data_archive/favrskov")) dir.create("../data_archive/favrskov_archive")


#Getting the main url link in
master_url <- "https://polweb.nethotel.dk/Produkt/PolWeb/default.asp?p=favrskov07"
  ...
  for(yc in 1:length(years)){
    ...
    if(length(links) >= 1){
      for (mc in 1:length(links)) {
        ...
        file.name <- paste0("../data_archive/favrskov_archive/", date, ".RData")
        ...
        if (file.exists(file.name)){
          ...
          junior_url2 <- paste0("HTTP://polweb.nethotel.dk/Produkt/PolWeb/Sog/ShowAgenda.asp?p=favrskov07&ID=",
...
save(out, file = '../data_archive/favrskov_07-17.RData')
